...
def load_topics() -> list[dict]:
    try:
        ...
        return [
            {"id": "static-la", "name": "linear_algebra", "question_count": 0},
...
if "turn_idx" not in st.session_state:
    st.session_state.turn_idx = 0  # для безопасной очистки инпута
...
mode = st.sidebar.selectbox(
    "Режим",
    ["exam", "diagnostic"],
    help="В 'exam' — ровно 10 вопросов. В 'diagnostic' — без лимита.",
)
...
with col1:
    ...
    if st.session_state.session_id:
        user_key = f"user_input_{st.session_state.turn_idx}"
        user_inp = st.text_input("Ваш ответ:", key=user_key)
        if st.button("Отправить") and user_inp.strip():
            ...
            # вместо прямой модификации значения виджета — меняем ключ
